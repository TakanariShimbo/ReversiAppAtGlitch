...
socket.on('game_start', function (data) {
    const turn_color = data.turn_color;
    updateMessageByTurn(turn_color);
});
...
socket.on('update_board', function (data) {
    ...
    for (let i = 0; i < xy_flips.length; i++) {
        setTimeout(function() {
            updateStoneMeshByBoard(xy_flips[i][0], xy_flips[i][1], current_board);
        }, 150*(i+1));
    }
    ...
    setTimeout(function() {
        for (let i = 0; i < xy_candidates.length; i++) {
            updateStoneMeshByBoard(xy_candidates[i][0], xy_candidates[i][1], current_board);
        }
